...
def build_fp(cfg):
    ...
    assert(m_specaug.bypass==False) # Detachable by setting m_specaug.bypass. This will be useful for tf.Model.fit() users.
...
def mini_search_validation(ds, m_pre, m_fp, mode='argmin',
    ...
    for k in key_strs:
        (db[k], query[k]) = (tf.zeros((0, dim[k])), tf.zeros((0, dim[k]))) # Initiazlie with empty tensors
...
def trainer(cfg, checkpoint_name):
    ...
    for ep in range(ep_start, ep_max + 1):
        ...
        if cfg['TRAIN']['MINI_TEST_IN_TRAIN']:
            accs_by_scope, scopes, key_strs = mini_search_validation(val_ds, m_pre, m_fp)
            for k in key_strs:
